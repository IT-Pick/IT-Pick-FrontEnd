    ...
    <div className={`flex pb-[20px] ${className}`}>
      {editMode && (
        <button onClick={() => toggleSelect(id)} className="mr-4">
          <img
            src={selectedItems.includes(id) ? ico_roundcheck : ico_rounduncheck}
            alt="select_icon"
          />
        </button>
      )}
...
const Debate: React.FC = () => {
  ...
  const handleDeleteClick = async () => {
    try {
      ...
      setDebates(
        debates.filter((debate) => !selectedItems.includes(debate.debateId))
      );
      setSelectedItems([]);
      setIsEditMode(false);
      console.log('삭제 성공');
      ...
      <header className="w-full flex justify-between items-center py-4">
        ...
        <div className="flex items-center">
          ...
          {!isEditMode && (
            <p
              onClick={toggleEditMode}
              className="text-point400 text-[14px] font-[500] mr-[24px] cursor-pointer"
            >
              편집
            </p>
          )}
