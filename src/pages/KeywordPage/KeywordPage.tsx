...
const KeywordPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAndSetKeywordData = async () => {
      ...
      try {
        const result = await getKeywordRelatedData(
          'total',
          'realtime',
          selectedKeyword
        );
  ...
  return (
    <div className="w-custom max-w-custom mx-auto bg-[#F8F9FC] min-h-screen">
      <Header />
      {keywordData && (
        <>
          <MainCard
      ...
      {keywordId && <LiveDiscussion keywordId={keywordId} />}{' '}
      {/* keywordId를 전달 */}
      <button
        className="fixed bottom-[18px] right-0"
